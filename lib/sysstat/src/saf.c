...
extern struct report_format json_fmt;
...
/*
 ***************************************************************************
 * Display a restart message (contents of a R_RESTART record). in JSON (DEL)
 *
 * IN:
 ...
 ***************************************************************************
 */
__printf_funct_t print_saf_restart(int *tab, int action, char *cur_date, char *cur_time,
...
/*
 ***************************************************************************
 * Display a comment (contents of R_COMMENT record). in JSON (DEL)
 *
 * IN:
 * @tab		Number of tabulations (unused here).
 * @action	Action expected from current function (unused here).
 ...
 ***************************************************************************
 */
__print_funct_t print_saf_comment(int *tab, int action, char *cur_date, char *cur_time,
		...
		*eosaf = read_record_hdr(ifd, rec_hdr_tmp, &record_hdr[*curr],
		...
		if (rtype != R_COMMENT) {
			...
			next = print_special_record(&record_hdr[*curr], flags,
						    ...
						    &rectime, file, 0, NULL,
						    file_magic, &file_hdr, act, &json_fmt,
	...
	do {
		...
		do {
			if (read_record_hdr(ifd, rec_hdr_tmp, &record_hdr[0], &file_hdr,
					    arch_64, endian_mismatch, UEOF_STOP, sizeof(rec_hdr_tmp), flags, &json_fmt)) {
				/* End of sa data file */
				return;
			}
			...
			if ((rtype == R_RESTART) || (rtype == R_COMMENT)) {
				print_special_record(&record_hdr[0], flags,
						     ...
						     &file_hdr, act, &json_fmt, endian_mismatch, arch_64);
			}
			else {
				/*
		...
		if (cnt == 0) {
			...
			do {
				...
				eosaf = read_record_hdr(ifd, rec_hdr_tmp, &record_hdr[curr],
							&file_hdr, arch_64, endian_mismatch, UEOF_STOP, sizeof(rec_hdr_tmp), flags, &json_fmt);
				rtype = record_hdr[curr].record_type;
